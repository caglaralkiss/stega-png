...
export async function hide(argv: HideArgv): Promise<void> {
	...
	if (fileByte * 3 > imgByte) {
		console.log(chalk.red(`Image size for this file must be at least ${fileByte * 3} byte!`))
		return
	}
	...
	console.log(chalk.yellow.italic(`Injecting the ${argv.textfile} content to ${argv.destination}`))

	const stegoBuffer = inject(file, opacityFiltered)
	const stegoBufferWithOpacity = populateWithOpacityValues(pixels, stegoBuffer)
	...
	console.log(chalk.green('Stego buffer is successfully created at ' + argv.destination))
}
